...
- Locate and decode PNG images and JPEG photos on rectangular or offset
  canvases.
...
4. Drop a generated SVG, PNG, JPG, or JPEG into the decoder panel to verify
   and recover its payload.

The decoder operates locally in the browser. Imported files are limited to
64 MiB by the application.

## Reliable image and photo decoding

The raster sampler accepts lossless PNG files and JPEG photos. It automatically
handles:
...
The sampler uses a small spatial vote around each expected cell center to
reduce isolated compression and sensor artifacts. It fits the projected outer
circle as a general ellipse, uses the isolated center finder to recover the
...
| `lib/rob-code-v2-raster-sampler.js` | PNG/JPEG localization, projective rectification, and sampling |
...
Decode browser `ImageData` obtained from a PNG or JPEG:
...
affine/projective rectification, photo-like artifacts, rotation, mirroring,
PNG export, JPEG import, and UI integration.

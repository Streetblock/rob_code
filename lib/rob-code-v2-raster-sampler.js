(function (root, factory) {
  ...
  class RoBCode2RasterSampler {
    ...
    decodeImageData(imageData, options = {}) {
      ...
      let background = this.estimateBackground(imageData);
      let contrast = this.maximumColorDistance(imageData, background);
      ...
      let threshold = Math.max(settings.minimumColorDistance, contrast * settings.thresholdRatio);
      let localization;
      try {
        localization = this.locateSymbol(
          imageData,
          background,
          threshold,
        ...
        if (!(error instanceof RoBCode2RasterError) || error.code !== "LOCALIZATION") throw error;
        const scene = this.locateSymbolInScene(imageData, settings);
      ...
      for (const candidate of candidates) {
        try {
          ...
          const erasureByteIndices = this.findFrameErasures(
            candidate,
            localization,
            isDark
          );
          const decoded = this.decoder.decodeCells(cells, erasureByteIndices, {
          ...
          const result = {
            ...
            rasterMetadata: {
              ...
              detectedErasures: erasureByteIndices.length,
              sceneLocalized: Boolean(localization.sceneLocalized),
          ...
          const hasSamplingWarnings = result.correctedSymbols > 0
            || result.erasureSymbols > 0
            || result.parityFailures.length > 0
            || result.paddingFailures.length > 0;
          if (!hasSamplingWarnings) return result;
      ...
      if (successfulResults.length) {
        successfulResults.sort((left, right) => (
          this.resultQualityCost(left) - this.resultQualityCost(right)
        ));
    ...
    resultQualityCost(result) {
      return result.correctedSymbols * 4
        + result.erasureSymbols * 0.5
        + result.parityFailures.length
        + result.paddingFailures.length
        + result.rasterMetadata.syncMismatches * 8
        + result.rasterMetadata.structureMismatches;
    }
    ...
    findCandidates(localization, isDark, settings) {
      const candidates = [];
      const outerRadiusOffset = localization.outerRadiusOffset || 1.35;
      const maximumByResolution = Math.floor(
        localization.minorRadius / (settings.minimumModulePixels * 0.95) - outerRadiusOffset
      );
      ...
      for (let outerDataRing = MIN_OUTER_RING; outerDataRing <= maximumRing; outerDataRing++) {
        const candidate = {
          outerDataRing,
          outerRadiusOffset,
          moduleSizeMajor: localization.majorRadius / (outerDataRing + outerRadiusOffset),
          moduleSizeMinor: localization.minorRadius / (outerDataRing + outerRadiusOffset)
        };
        ...
        const maximumStructureMismatches = localization.touchesImageEdge ? 8 : 4;
        if (structureMismatches > maximumStructureMismatches) continue;
    ...
    findFrameErasures(candidate, localization, isDark) {
      const erasures = [];
      const sampleRadius = candidate.moduleSize * 0.04;
      let byteOffset = 0;
      for (let ring = 2; ring <= candidate.outerDataRing; ring++) {
        ...
        for (let byte = 0; byte < byteCount; byte++) {
          ...
          for (const fraction of [0.25, 0.5, 0.75]) {
            const logicalAngle = (byte + fraction) * 360 / byteCount;
            const imageAngle = candidate.rotationDegrees + candidate.direction * logicalAngle;
            const point = this.mapEllipsePoint(
              localization,
              candidate,
              candidate.outerDataRing + 1.25,
              imageAngle
            );
            if (isDark(point.x, point.y, sampleRadius)) darkProbes += 1;
          }
          if (darkProbes < 2) erasures.push(byteOffset + byte);
        }
    ...
    locateSymbolInScene(imageData, settings) {
      const width = imageData.width;
      const height = imageData.height;
      const pixelCount = width * height;
      if (pixelCount > 16000000) {
        throw new RoBCode2RasterError(
          "PHOTO_SIZE",
          "Photo scene localization supports at most 16 million pixels"
        );
      }
      ...
      for (let index = 0; index < pixelCount; index++) {
        ...
        if (luminance < 115 && chroma < 58) darkMask[index] = 1;
      }
      ...
      for (let start = 0; start < pixelCount; start++) {
        ...
        while (head < length) {
          ...
          for (let offsetY = -1; offsetY <= 1; offsetY++) {
            for (let offsetX = -1; offsetX <= 1; offsetX++) {
              ...
              if (!darkMask[next] || visited[next]) continue;
              visited[next] = 1;
              queue[length++] = next;
            }
          }
        ...
        const fillRatio = length / (componentWidth * componentHeight);
        if (length < 200
          || componentWidth < 64
          || componentHeight < 64
          || fillRatio < 0.003
        ...
        const originX = (left + right + 1) / 2;
        const originY = (top + bottom + 1) / 2;
        const coordinateScale = Math.hypot(componentWidth, componentHeight);
        const pointStep = Math.max(1, Math.ceil(length / 50000));
        const points = [];
        for (let index = 0; index < length; index += pointStep) {
          const position = queue[index];
          const x = position % width;
          const y = Math.floor(position / width);
          points.push({ x: x + 0.5 - originX, y: y + 0.5 - originY });
        }

        try {
          const ellipse = this.fitEllipsePoints(points, originX, originY, coordinateScale, 0.12);
          const axisRatio = ellipse.minorRadius / ellipse.majorRadius;
          if (axisRatio < settings.minimumAxisRatio || ellipse.minorRadius < 32) continue;
          if (ellipse.ellipseCenterX < left || ellipse.ellipseCenterX > right
            || ellipse.ellipseCenterY < top || ellipse.ellipseCenterY > bottom) {
            continue;
          }
          const coveredAngles = new Uint8Array(72);
          for (const point of points) {
            const normalized = this.toEllipseCoordinates(
              ellipse,
              originX + point.x,
              originY + point.y
            );
            const radius = Math.hypot(normalized.x, normalized.y);
            if (radius < 0.82 || radius > 1.18) continue;
            const angle = (Math.atan2(normalized.y, normalized.x) + 2 * Math.PI) % (2 * Math.PI);
            coveredAngles[Math.floor(angle / (2 * Math.PI) * coveredAngles.length)] = 1;
          }
          const coverage = coveredAngles.reduce((sum, value) => sum + value, 0) / coveredAngles.length;
          const touchesImageEdge = left <= 1
            || top <= 1
            || right >= width - 2
            || bottom >= height - 2;
          const minimumCoverage = touchesImageEdge ? 0.6 : 0.82;
          if (coverage < minimumCoverage) continue;
          ellipseCandidates.push({
            ...ellipse,
            left,
            top,
            right,
            ...
            coverage,
            touchesImageEdge,
            score: ellipse.ellipseResidual + (1 - coverage) * 0.25
          });
        } catch (error) {
          // Non-elliptical dark components are expected in ordinary photographs.
        }
      }

      ellipseCandidates.sort((left, right) => left.score - right.score);
      for (const ellipse of ellipseCandidates) {
        const backgroundSamples = [];
        for (let angle = 0; angle < 360; angle += 4) {
          const point = this.mapNormalizedEllipsePoint(ellipse, 1.06, angle);
          if (point.x < 0 || point.y < 0 || point.x >= width || point.y >= height) continue;
          backgroundSamples.push(this.sampleColor(imageData, point.x, point.y));
        }
        if (backgroundSamples.length < 45) continue;
        const background = [0, 1, 2].map(channel => this.median(
          backgroundSamples.map(color => color[channel])
        ));
        let contrast = 0;
        for (let angle = 0; angle < 360; angle += 2) {
          const point = this.mapNormalizedEllipsePoint(ellipse, 1, angle);
          contrast = Math.max(
            contrast,
            this.colorDistance(this.sampleColor(imageData, point.x, point.y), background)
          );
        }
        if (contrast < settings.minimumContrast) continue;
        const threshold = Math.max(
          settings.minimumColorDistance,
          contrast * settings.thresholdRatio
        );
        try {
          const centerFinder = this.locateCenterFinder(
            imageData,
          ...
          const centerInEllipse = this.toEllipseCoordinates(ellipse, centerFinder.x, centerFinder.y);
          const perspectiveStrength = Math.hypot(centerInEllipse.x, centerInEllipse.y);
          if (perspectiveStrength >= 0.82) continue;
          return {
            background,
            contrast,
            threshold,
            localization: {
              ...
              projectiveCenter: centerInEllipse,
              perspectiveStrength,
              outerRadiusOffset: 1.25,
              sceneLocalized: true
            }
          };
        } catch (error) {
          // Try the next complete elliptical dark component.
        }
      }

      throw new RoBCode2RasterError(
        "LOCALIZATION",
        "No complete RoBCode 2 outer frame could be isolated in the photo"
      );
    }

    mapNormalizedEllipsePoint(ellipse, radius, angleInDegrees) {
      const angle = angleInDegrees * Math.PI / 180;
      const normalizedX = radius * Math.cos(angle);
      const normalizedY = radius * Math.sin(angle);
      return {
        x: ellipse.ellipseCenterX
          + ellipse.majorVector[0] * normalizedX * ellipse.majorRadius
          - ellipse.majorVector[1] * normalizedY * ellipse.minorRadius,
        y: ellipse.ellipseCenterY
          + ellipse.majorVector[1] * normalizedX * ellipse.majorRadius
          + ellipse.majorVector[0] * normalizedY * ellipse.minorRadius
      };
    }
    ...
    fitEllipsePoints(points, originX, originY, coordinateScale, maximumResidual) {
      ...
      const ellipseCenterX = originX + localEllipseCenterX * coordinateScale;
      const ellipseCenterY = originY + localEllipseCenterY * coordinateScale;
    ...
    mapEllipsePoint(localization, candidate, radius, angleInDegrees) {
      ...
      const normalizedRadius = radius / (
        candidate.outerDataRing + (candidate.outerRadiusOffset || 1.35)
      );

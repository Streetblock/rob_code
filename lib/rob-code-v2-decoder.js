(function (root, factory) {
  ...
  class RoBCode2Decoder {
    ...
    decodeCells(input, erasureByteIndices = [], options = {}) {
      const cells = this.copyCells(input);
      const allowDamagedPadding = options.allowDamagedPadding === true;
      if (cells.length % 9 !== 0) {
      ...
      const paddingFailures = [];
      padding.forEach((byte, index) => {
        const byteIndex = codeStreamLength + index;
        if (byte !== 0 && !erasureByteIndices.includes(byteIndex)) {
          paddingFailures.push(byteIndex);
        }
      });
      if (paddingFailures.length && !allowDamagedPadding) {
